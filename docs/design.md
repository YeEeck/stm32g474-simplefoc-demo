...
- **采样时刻**：中心对齐模式中点（计数器下溢/上溢附近，占空比中央），采样时刻电流纹波最小
- **增益**：DRV8316CT CSA 增益由板上引脚固定（假设 datasheet 默认档 1.45 V/A，T 版具体取值以实际板为准）。换算：`I = V_adc / gain`；**实现阶段必须标定**（见 §7）
- **信号特性**：IPROPI 输出为内部滤波后的平均电流，带宽有限。缓解措施：
