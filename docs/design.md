...
- **增益**：DRV8316CT 为引脚配置版，CSA 增益由 **GAIN 引脚**电阻决定（四电平）：
  ...
  SOx 输出 = VREF/2 + I×GCSA；VREF 引脚由板上供电（2.8V~AVDD，外接 0.1µF）。电机额定 1.4A/堵转 4.9A，**推荐 0.3~0.6 V/A 档**（1.2 档量程不够，0.15 档分辨率差）。
  换算：`I = (V_sox - VREF/2) / GCSA`；**固件当前按 0.6 V/A 配置，上板前必须确认板上 GAIN 引脚接法并标定**（§7 步骤 3）
