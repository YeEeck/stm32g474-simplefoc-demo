...
4. `motor.initFOC()`：**电压对齐校准**——施加固定电压矢量锁转子，标定编码器方向与电角零位；随后电流采样相序自校正（`driverAlign`，SOx 接反时自动交换通道映射，`Success: 2/3/4`）。Z 索引不参与对齐（电压对齐已定零位，Z 脉冲被忽略）
5. 进入主循环：Commander 轮询 + `motor.move()` + `motor.monitor()`（每 100 次主循环一行 `target \t Vq \t Iq \t velocity \t angle`），默认目标电流 0，等待 `T` 指令

异常路径：对齐失败（编码器读数无变化）→ 打印错误 → 停止 PWM 输出 + nSLEEP 拉低（驱动板 sleep，无锁定电流）→ 死循环停机。
